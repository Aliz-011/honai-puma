...
import { subMonths, intlFormat, subDays, subYears, getDaysInMonth, format, endOfMonth } from "date-fns";
...
export const TableData = () => {
    ...
    const { data: revenues, isLoading: isLoadingRevenue } = useGetPayingSubs({ date: selectedDate })
    ...
    if (!revenues) {
        return (
            <TableNotFound date={selectedDate} daysBehind={3} tableName='Paying Subs' />
        )
    }

    const handleDownload = () => {
        exportToExcel(revenues, 'Paying Subs', selectedDate, compactDate)
    }
    ...
    return (
        <div className="w-fit overflow-x-auto remove-scrollbar">
            <ComponentCard title='Paying Subs' desc={<button
