...
const TrxNewSalesPage = () => {
    ...
    const { data, isLoading } = useGetTrxNewSales({ date: selectedDate })
    ...
    return (
        <div>
            ...
            <div className="overflow-hidden min-h-screen rounded-2xl border border-gray-200 bg-white px-5 py-7 dark:border-gray-800 dark:bg-white/[0.03] space-y-4">
                <Filters />
                <TableData data={data} latestUpdatedData={2} selectedDate={selectedDate} title="Trx New Sales" isLoading={isLoading} />
            </div>
        </div>
    )
}
