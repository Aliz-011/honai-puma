...
import { branches, clusters, kabupatens, regionals, subbranches, trxNewSales, trxNewSalesByu, trxNewSalesPrabayar } from "@/db/schema";
...
const app = new Hono()
    .get('/', zValidator('query', z.object({ date: z.coerce.date().optional() })),
        async c => {
            ...
            const monthColumn = `m${month}` as keyof typeof trxNewSales.$inferSelect
            ...
            const sq4 = db2
                .select({
                    ...
                    trx: prevYearCurrMonthRevNewSales.trx,
            ...
            const p1 = db
                .select({
                    ...
                    currMonthTargetRev: sql<number>`CAST(SUM(${trxNewSales[monthColumn]}) AS DOUBLE PRECISION)`.as('currMonthTargetRev')
                })
                .from(regionals)
                ...
                .leftJoin(trxNewSales, eq(kabupatens.id, trxNewSales.kabupatenId))
                .groupBy(
                    regionals.regional,
                    branches.branchNew,
            ...
            const queryCurrYtd = currYtdRevNewSales.map(table => `
                    SELECT
                        ...
                        mtd_dt,
                        trx
                    FROM ${table} WHERE cat = 'new_sales'`).join(' UNION ALL ')
            ...
            const sq = `
                    ...
                    FROM sq
                    WHERE kabupaten <> 'TMP' AND mtd_dt BETWEEN '${currJanuaryFirst}' AND '${currDate}'
                    GROUP BY 1, 2, 3, 4, 5
                        `
            ...
            const sq5 = `
                    ...
                    FROM sq5
                    WHERE kabupaten <> 'TMP' AND mtd_dt BETWEEN '${prevJanuaryFirst}' AND '${prevYearCurrDate}'
                    GROUP BY 1, 2, 3, 4, 5
                        `
    ...
    .get('/trx-new-sales-prabayar', zValidator('query', z.object({ date: z.coerce.date().optional() })),
        async c => {
            ...
            // KOLOM DINAMIS UNTUK MEMILIH ANTARA KOLOM `m1-m12`
            const monthColumn = `m${month}` as keyof typeof trxNewSalesPrabayar.$inferSelect
            ...
            const p1 = db
                .select({
                    ...
                    cluster: clusters.cluster,
                    kabupaten: kabupatens.kabupaten,
                    currMonthTargetRev: sql<number>`CAST(SUM(${trxNewSalesPrabayar[monthColumn]}) AS DOUBLE PRECISION)`.as('currMonthTargetRev')
                })
                .from(regionals)
                .leftJoin(branches, eq(regionals.id, branches.regionalId))
                .leftJoin(subbranches, eq(branches.id, subbranches.branchId))
                .leftJoin(clusters, eq(subbranches.id, clusters.subbranchId))
                .leftJoin(kabupatens, eq(clusters.id, kabupatens.clusterId))
                .leftJoin(trxNewSalesPrabayar, eq(kabupatens.id, trxNewSalesPrabayar.kabupatenId))
                .groupBy(
                    regionals.regional,
                    branches.branchNew,
                    subbranches.subbranchNew,
            ...
            const queryCurrYtd = currYtdRevNewSales.map(table => `
                    SELECT
                        ...
                        mtd_dt,
                        trx
                    FROM ${table} WHERE brand <> 'ByU' AND cat = 'new_sales'`).join(' UNION ALL ')
    ...
    .get('/trx-new-sales-byu', zValidator('query', z.object({ date: z.coerce.date().optional() })),
        async c => {
            const { date } = c.req.valid('query')
            const selectedDate = date ? new Date(date) : subDays(new Date(), 2)
            const month = (selectedDate.getMonth() + 1).toString()
            ...
            // KOLOM DINAMIS UNTUK MEMILIH ANTARA KOLOM `m1-m12`
            const monthColumn = `m${month}` as keyof typeof trxNewSalesByu.$inferSelect
            ...
            const sq3 = db2
                ...
                .where(and(
                    ...
                    and(
                        not(eq(prevMonthRevNewSales.kabupaten, 'TMP')),
                        eq(prevMonthRevNewSales.brand, 'ByU')
                    ),
                    and(
                        gte(prevMonthRevNewSales.mtdDt, firstDayOfPrevMonth),
                        lte(prevMonthRevNewSales.mtdDt, prevDate)
            ...
            const sq4 = db2
                ...
                .where(and(
                    ...
                    and(
                        not(eq(prevYearCurrMonthRevNewSales.kabupaten, 'TMP')),
                        eq(prevYearCurrMonthRevNewSales.brand, 'ByU')
                    ),
                    and(
                        gte(prevYearCurrMonthRevNewSales.mtdDt, firstDayOfPrevYearCurrMonth),
                        lte(prevYearCurrMonthRevNewSales.mtdDt, prevYearCurrDate)
            ...
            const p1 = db
                .select({
                    ...
                    cluster: clusters.cluster,
                    kabupaten: kabupatens.kabupaten,
                    currMonthTargetRev: sql<number>`CAST(SUM(${trxNewSalesByu[monthColumn]}) AS DOUBLE PRECISION)`.as('currMonthTargetRev')
                })
                .from(regionals)
                .leftJoin(branches, eq(regionals.id, branches.regionalId))
                .leftJoin(subbranches, eq(branches.id, subbranches.branchId))
                .leftJoin(clusters, eq(subbranches.id, clusters.subbranchId))
                .leftJoin(kabupatens, eq(clusters.id, kabupatens.clusterId))
                .leftJoin(trxNewSalesByu, eq(kabupatens.id, trxNewSalesByu.kabupatenId))
                .groupBy(
                    regionals.regional,
                    branches.branchNew,
                    subbranches.subbranchNew,
