...
const app = new Hono().get("/",
  ...
  async (c) => {
    ...
    const p1 = db
      ...
      .from(regionals)
      .innerJoin(branches, eq(regionals.id, branches.regionalId))
      .innerJoin(subbranches, eq(branches.id, subbranches.branchId))
      .innerJoin(clusters, eq(subbranches.id, clusters.subbranchId))
      .innerJoin(kabupatens, eq(clusters.id, kabupatens.clusterId))
      .innerJoin(revenueGrosses, eq(kabupatens.id, revenueGrosses.kabupatenId))
      .groupBy(
        regionals.regional,
        branches.branchNew,
    ...
    console.log({ rvp: { firstDayOfCurrMonth, currDate }, prvp: { firstDayOfPrevMonth, prevDate }, pyrvp: { firstDayOfPrevYearCurrMonth, prevYearCurrDate } });
    ...
    const p2 = db2
      ...
      .from(rvp)
      .where(sql`(${rvp.branch} IS NOT NULL AND ${rvp.subbranch} IS NOT NULL) AND ${rvp.mtdDt} BETWEEN ${sql.placeholder('firstDayOfCurrMonth')} AND ${sql.placeholder('currDate')}`)
    ...
    const p3 = db2
      ...
      .from(prvp)
      .where(sql`
        (${prvp.branch} IS NOT NULL AND ${prvp.subbranch} IS NOT NULL) AND ${prvp.mtdDt} BETWEEN ${sql.placeholder('firstDayOfPrevMonth')} AND ${sql.placeholder('prevDate')}
      `)
    ...
    const p4 = db2
      ...
      .from(pyrvp)
      .where(sql`
          (${pyrvp.branch} IS NOT NULL AND ${pyrvp.subbranch} IS NOT NULL) AND ${pyrvp.mtdDt} BETWEEN ${sql.placeholder('firstDayOfPrevYearCurrMonth')} AND ${sql.placeholder('prevYearCurrDate')}
          `)
    ...
    const [targetRevenue, currMonthRevenue, prevMonthRevenue, prevYearCurrMonthRevenue] = await Promise.all([
      p1.execute(),
      p2.execute({ firstDayOfCurrMonth, currDate }),
      p3.execute({ prevDate, firstDayOfPrevMonth }),
      p4.execute({ prevYearCurrDate, firstDayOfPrevYearCurrMonth })
    ]);
